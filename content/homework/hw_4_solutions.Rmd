...
The $p$-value of the test is $P\left(\left|t\right| > \left|t^*\right| \right) = `r 2*pt(abs(summary(linmod)$coef["X", "t value"]), df = length(Y) - 2, lower.tail = FALSE)`$, the probability that a $t$-random variable with $82$ degrees of freedom is greater than $\left|t^*\right|$ in absolute value. 
